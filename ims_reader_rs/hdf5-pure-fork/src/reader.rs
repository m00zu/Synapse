...
impl<'f> Dataset<'f> {
    ...
    pub fn read_u16(&self) -> Result<Vec<u16>, Error> {
        let raw = self.read_raw()?;
        let dt = self.datatype()?;
        // Fast path: native little-endian u16 — zero-copy reinterpret
        let elem_size = data_read::get_size(&dt);
        if elem_size == 2 && cfg!(target_endian = "little") {
            let count = raw.len() / 2;
            let mut result = vec![0u16; count];
            // Safety: raw is aligned to u8, but we copy via from_le_bytes
            for i in 0..count {
                result[i] = u16::from_le_bytes([raw[i * 2], raw[i * 2 + 1]]);
            }
            return Ok(result);
        }
    ...
    /// Read raw bytes of the dataset (public for direct access).
    pub fn read_raw_bytes(&self) -> Result<Vec<u8>, Error> {
        self.read_raw()
    }
