...
// ═══════════════════════════════════════════════════════════════════════════
//  Float32 morphology (for the float [0,1] pipeline)
// ═══════════════════════════════════════════════════════════════════════════

fn erode_impl_f32(
    image: &[f32], height: usize, width: usize, kernel: &[u8], ksize: usize,
) -> Vec<f32> {
    let npix = height * width;
    ...
    out.par_chunks_mut(width).enumerate().for_each(|(y, row_out)| {
        for x in 0..width {
            ...
            for ky in 0..ksize {
                for kx in 0..ksize {
                    if kernel[ky * ksize + kx] == 0 { continue; }
                    let iy = y as isize + ky as isize - half as isize;
                    let ix = x as isize + kx as isize - half as isize;
                    let pix = if iy >= 0 && iy < height as isize && ix >= 0 && ix < width as isize {
                        image[iy as usize * width + ix as usize]
                    } else { 1.0 }; // border = max for erosion
        ...
        for x in 0..width {
            ...
            for ky in 0..ksize {
                for kx in 0..ksize {
                    ...
                    let pix = if iy >= 0 && iy < height as isize && ix >= 0 && ix < width as isize {
                        image[iy as usize * width + ix as usize]
                    } else { 0.0 }; // border = 0 for dilation
...
/// Input: grayscale f32 image [0,1], u32 marker array, optional u8 mask.
/// Returns: u32 label array.
#[pyfunction]
#[pyo3(signature = (image_np, markers_np, mask_np=None))]
fn watershed<'py>(
    py: Python<'py>,
    image_np: PyReadonlyArray2<'py, f32>,
...
/// Grayscale erosion with disk structuring element (float32 [0,1]).
#[pyfunction]
fn erode<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let result = py.detach(move || erode_impl_f32(&data, height, width, &kernel, ksize));
    vec_to_pyarray2(py, result, height, width)
}

/// Grayscale dilation with disk structuring element (float32 [0,1]).
#[pyfunction]
fn dilate<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let data = img_np
        ...
        .to_vec();
    let (kernel, ksize) = disk_kernel_flat(radius);
    let result = py.detach(move || dilate_impl_f32(&data, height, width, &kernel, ksize));
    vec_to_pyarray2(py, result, height, width)
}

/// Morphological opening (erode then dilate, float32).
#[pyfunction]
fn opening<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let result = py.detach(move || {
        let eroded = erode_impl_f32(&data, height, width, &kernel, ksize);
        dilate_impl_f32(&eroded, height, width, &kernel, ksize)
    });
    vec_to_pyarray2(py, result, height, width)
}

/// Morphological closing (dilate then erode, float32).
#[pyfunction]
fn closing<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let (kernel, ksize) = disk_kernel_flat(radius);
    let result = py.detach(move || {
        let dilated = dilate_impl_f32(&data, height, width, &kernel, ksize);
        erode_impl_f32(&dilated, height, width, &kernel, ksize)
    });
    vec_to_pyarray2(py, result, height, width)
}

/// White top-hat: original - opening (float32).
#[pyfunction]
fn white_tophat<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let result = py.detach(move || {
        let eroded = erode_impl_f32(&data, height, width, &kernel, ksize);
        let opened = dilate_impl_f32(&eroded, height, width, &kernel, ksize);
        data.iter().zip(opened.iter()).map(|(&o, &op)| (o - op).max(0.0)).collect::<Vec<f32>>()
    });
    vec_to_pyarray2(py, result, height, width)
}

/// Black top-hat: closing - original (float32).
#[pyfunction]
fn black_tophat<'py>(
    py: Python<'py>,
    img_np: PyReadonlyArray2<'py, f32>,
    ...
    let result = py.detach(move || {
        let dilated = dilate_impl_f32(&data, height, width, &kernel, ksize);
        let closed = erode_impl_f32(&dilated, height, width, &kernel, ksize);
        closed.iter().zip(data.iter()).map(|(&c, &o)| (c - o).max(0.0)).collect::<Vec<f32>>()
    });
    vec_to_pyarray2(py, result, height, width)
}

/// CLAHE — Contrast Limited Adaptive Histogram Equalization.
/// Input: 2D float32 grayscale image [0,1], tile_size (e.g. 8), clip_limit (e.g. 2.0).
/// Returns: enhanced 2D float32 image [0,1].
#[pyfunction]
...
fn clahe<'py>(
    ...
    let data_f32 = img_np
        ...
        .map_err(|_| PyValueError::new_err("Array must be contiguous"))?
        .to_vec();
    // Quantize f32 [0,1] → u8 [0,255] for CLAHE (histogram-based), convert back
    let result = py.detach(move || {
        let data_u8: Vec<u8> = data_f32.iter()
            .map(|&v| (v.clamp(0.0, 1.0) * 255.0) as u8)
            .collect();
        let out_u8 = clahe_impl(&data_u8, height, width, tile_size, clip_limit);
        out_u8.iter().map(|&v| v as f32 / 255.0).collect::<Vec<f32>>()
    });
    vec_to_pyarray2(py, result, height, width)
}
