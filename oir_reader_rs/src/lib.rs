...
fn read_oir_frames_tiled(
    ...
    // Integer-exact equivalent of (v / max_val * 255.0) as u8.
    // u32 headroom: 65535 * 255 = 16_711_425, well within u32::MAX.
    if bitsize > 32 {
        return Err(format!("Unsupported bit depth: {bitsize} (max 32)"));
    }
    let max_val_u32 = ((1u64 << bitsize) - 1) as u32;

    // Parallel: each channel fills its own flat row-major buffer independently.
    let channel_data: Vec<Vec<u8>> = (0..n_ch)
        .into_par_iter()
        .map(|ch| {
            let mut data         = vec![0u8; size_y * size_x];
            ...
            for j in 0..n_div {
                ...
                for row in 0..lines_this {
                    ...
                    for col in 0..cols {
                        ...
                        data[dst_off + col] = ((v as u32 * 255) / max_val_u32) as u8;
                    }
                }
                rows_written += lines_this;
            }
...
// 5.  read_single_oir
// ──────────────────────────────────────────────────────────────────────
fn read_single_oir(path: &Path) -> Res<(Array3<u8>, OirMeta)> {
...
struct RawOirResult {
    ...
    data:  Vec<u8>, // row-major: [h*w] or [h*w*n_out]
}
