...
from synapse.data_models import ImageData, MaskData, TableData, FigureData, LabelData
from synapse.nodes.base import PORT_COLORS, BaseExecutionNode
from synapse.nodes.base import BaseImageProcessNode, _arr_to_pil
...
def _get_image_arr(node, port_name: str):
    """Return (arr_uint8, pil, cls) from an image/mask input port, or (None, None, None)."""
    ...
    if isinstance(data, ImageData):
        return np.array(data.payload), data.payload, ImageData
    if isinstance(data, MaskData):
        return np.array(data.payload), data.payload, MaskData
    return None, None, None
...
def _get_mask_arr(node, port_name: str):
    ...
    data = cp.node().output_values.get(cp.name())
    if isinstance(data, MaskData):
        return np.array(data.payload.convert('L')) > 0
    return None
    ...
    def evaluate(self):
        ...
        if arr.dtype == np.uint8:
            try:
                ...
                if arr.ndim == 2:
                    result = np.asarray(_rs.white_tophat(arr, radius))
                    rs_ok = True
                elif arr.ndim == 3:
                    channels = [np.asarray(_rs.white_tophat(arr[:, :, c], radius))
        ...
        mode = 'L' if result.ndim == 2 else ('RGB' if result.shape[2] == 3 else 'RGBA')

        self.set_progress(80)
        pil_out = _arr_to_pil(result.astype(np.uint8), mode)
        self.output_values['image'] = out_cls(payload=pil_out)
        self.set_display(pil_out)
    ...
    def evaluate(self):
        ...
        if arr.dtype == np.uint8:
            try:
                import image_process_rs as _rs
                if arr.ndim == 2:
                    result = np.asarray(_rs.black_tophat(arr, radius))
                    rs_ok = True
                elif arr.ndim == 3:
                    channels = [np.asarray(_rs.black_tophat(arr[:, :, c], radius))
                                for c in range(arr.shape[2])]
                    result = np.stack(channels, axis=2)
                    rs_ok = True
            except Exception:
...
class BandpassFilterNode(BaseImageProcessNode):
    ...
    **Remove > (px)** — suppress structures larger than this value (low-pass cutoff). Set to `0` to disable low-pass (keep all large features).
    ...
    def evaluate(self):
        ...
        if arr.ndim == 3:
            gray = np.array(pil_in.convert('L'), dtype=np.float64)
        else:
        ...
        pil_out = _arr_to_pil(filtered, 'L')
        self.output_values['image'] = out_cls(payload=pil_out)
        self.set_display(pil_out)
        self.set_progress(100)
        return True, None
...
class WatershedNode(BaseImageProcessNode):
    ...
    def evaluate(self):
        ...
        if _has_rs:
            try:
                neg_dist_u8 = (255 - np.clip(dist / dist.max() * 255, 0, 255)).astype(np.uint8) if dist.max() > 0 else np.zeros_like(binary, dtype=np.uint8)
                labeled = np.asarray(
                    _rs.watershed(neg_dist_u8, markers.astype(np.uint32),
...
class BlobDetectNode(BaseImageProcessNode):
    ...
    **Threshold** — detection sensitivity; lower values find more blobs.
    ...
    def evaluate(self):
        ...
        gray = np.array(pil_in.convert('L'), dtype=np.float64) / 255.0
        ...
        # Draw overlay
        overlay = pil_in.convert('RGB').copy()
        ...
        pil_overlay = overlay
        self.output_values['overlay'] = ImageData(payload=pil_overlay)
        self.set_display(pil_overlay)
        self.set_progress(100)
        return True, None
...
class FrangiNode(BaseImageProcessNode):
    ...
    **Sigma Max** — largest vessel width scale to detect.
    ...
    def evaluate(self):
        ...
        result_u8 = (response * 255).astype(np.uint8)

        pil_out = _arr_to_pil(result_u8, 'L')
        self.output_values['image'] = ImageData(payload=pil_out)
        self.set_display(pil_out)
        self.set_progress(100)
        return True, None
...
class ColocalizationNode(BaseExecutionNode):
    ...
    Keywords: colocalization, coloc, pearson, manders, icq, 共定位, 強度, 相關性, 影像處理, 分析
    """
    __identifier__ = 'nodes.image_process.measure'
    NODE_NAME      = 'Colocalization'
    PORT_SPEC      = {'inputs': ['image', 'image', 'mask'], 'outputs': ['table', 'figure']}
    OUTPUT_COLUMNS = {
        'table': ['PCC_full', 'PCC_masked', 'MOC', 'M1', 'M2', 'ICQ', 'Otsu_ch1', 'Otsu_ch2']
    }
    ...
    def _read_gray(self, port_name: str):
        ...
        if isinstance(data, (ImageData, MaskData)):
            return np.array(data.payload.convert('L'), dtype=np.float64)
        return None
    ...
    def evaluate(self):
        ...
        moc   = float(np.sum(a * b) / denom) if denom > 0 else float('nan')

        # ── Manders M1 / M2 (Otsu thresholds) ─────────────────────────────
        from skimage.filters import threshold_otsu
        t1 = threshold_otsu(a.astype(np.uint8)) if a.max() > a.min() else 0.0
        t2 = threshold_otsu(b.astype(np.uint8)) if b.max() > b.min() else 0.0
        s_a = np.sum(a)
        s_b = np.sum(b)
        m1 = float(np.sum(a[b > t2]) / s_a) if s_a > 0 else float('nan')
        m2 = float(np.sum(b[a > t1]) / s_b) if s_b > 0 else float('nan')
        self.set_progress(50)
        ...
        row = {
            'PCC_full':   [round(pcc_full,   4)],
            'PCC_masked': [round(pcc_masked, 4)],
            'MOC':        [round(moc,        4)],
            'M1':         [round(m1,         4)],
            'M2':         [round(m2,         4)],
            'ICQ':        [round(icq,        4)],
            'Otsu_ch1':   [round(float(t1),  2)],
            'Otsu_ch2':   [round(float(t2),  2)],
        }
        ...
        flat_b  = b.ravel()
        if mask is not None:
            flat_a = flat_a[mask.ravel()]
            flat_b = flat_b[mask.ravel()]
        ...
        ax.set_title(f'PCC = {pcc_full:.3f}  |  M1={m1:.3f}  M2={m2:.3f}')
        for sp in ['top', 'right']:
...
class GLCMTextureNode(BaseImageProcessNode):
    ...
    **Grey Levels** — number of quantisation levels (fewer = faster, coarser).
    ...
    def evaluate(self):
        ...
        self.output_values['table'] = TableData(payload=pd.DataFrame(row))
        self.set_display(pil_in.convert('L') if arr.ndim == 2 else pil_in)
        self.set_progress(100)
        return True, None
...
class ImageStatsNode(BaseImageProcessNode):
    ...
    **Column Prefix** — optional string prepended to all column names.
    ...
    def evaluate(self):
        ...
        preview = pil_in if arr is not None else (
            mask.astype(np.uint8) * 255
        )
        if isinstance(preview, np.ndarray):
            from PIL import Image as _PIL
            preview = _PIL.fromarray(preview)
        self.set_display(preview.convert('L') if arr is not None and arr.ndim == 2 else preview)
        self.set_progress(100)
        return True, None
...
class _LineGraphicsView(QGraphicsView):
    ...
    def load_image(self, pil_image):
        """Replace the background image. Existing line is kept in place."""
...
class _NodeLineViewWidget(NodeBaseWidget):
    ...
    def load_image(self, pil_image):
        """Thread-safe: load a PIL image into the view."""
        if threading.current_thread() is threading.main_thread():
            self._view.load_image(pil_image)
        else:
            self._img_signal.emit(pil_image)
...
class IntensityProfileNode(BaseImageProcessNode):
    ...
    def _load_image_into_widget(self):
        ...
        data = cp.node().output_values.get(cp.name())
        if isinstance(data, ImageData):
            self._line_widget.load_image(data.payload)
    ...
    def evaluate(self):
        ...
        self._line_widget.load_image(pil_in)
...
class CannyEdgeNode(BaseImageProcessNode):
    ...
    **High Threshold** — upper bound for hysteresis thresholding.
    ...
    def evaluate(self):
        ...
        pil = _arr_to_pil(out, 'L')
        self.output_values['mask'] = MaskData(payload=pil)
        self.set_display(pil)
        self.set_progress(100)
        return True, None
...
class SobelEdgeNode(BaseImageProcessNode):
    ...
    def evaluate(self):
        ...
        scaled = ((mag - mn) / (mx - mn) * 255).astype(np.uint8) if mx > mn else np.zeros_like(mag, dtype=np.uint8)
        pil = _arr_to_pil(scaled, 'L')
...
class LaplacianEdgeNode(BaseImageProcessNode):
    ...
    **Sigma** — spatial scale of the Gaussian smoothing before the Laplacian.
    ...
    def evaluate(self):
        ...
        if ext > 0:
            scaled = ((log / ext * 127) + 128).clip(0, 255).astype(np.uint8)
        else:
            scaled = np.full(log.shape, 128, dtype=np.uint8)
        pil = _arr_to_pil(scaled, 'L')
        self.output_values['image'] = ImageData(payload=pil)
        self.set_display(pil)
        self.set_progress(100)
...
class HoughCirclesNode(BaseImageProcessNode):
    ...
    **Threshold** — fraction of the peak accumulator value required for a detection.
    ...
    def evaluate(self):
        ...
        self.output_values['overlay'] = ImageData(payload=pil)
        self.set_display(pil)
        self.set_progress(100)
        return True, None
    ...
    def evaluate(self):
        ...
        if mask_port and mask_port.connected_ports():
            ...
            if mval is not None:
                ...
                m_np = np.array(mval.payload) if isinstance(mval, MaskData) else np.array(mval)
                if m_np.shape[:2] != img_np.shape[:2]:
                    from PIL import Image as _PIL
                    m_pil = _PIL.fromarray(m_np.astype(np.uint8))
                    m_pil = m_pil.resize((img_np.shape[1], img_np.shape[0]), _PIL.NEAREST)
                    m_np  = np.array(m_pil)
                bool_mask = m_np > 0
        ...
        max_pixel = int(img_np.max())
        if max_pixel <= 255:    range_max = 256
        elif max_pixel <= 4095: range_max = 4096
        else:                   range_max = 65536
        ...
        pixel_vals = (bin_edges[:-1] + bin_edges[1:]) / 2.0
        hist_dict  = {'Pixel_Value': pixel_vals}
        ...
        if is_rgb:
            ...
            for i, name in enumerate(ch_names):
                ...
                h, _ = np.histogram(px, bins=bins, range=(0, range_max))
                hist_dict[name] = h
