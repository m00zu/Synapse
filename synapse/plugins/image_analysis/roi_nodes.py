...
from synapse.nodes.base import BaseExecutionNode, PORT_COLORS
...
def _draw_styled_polyline(
    ...
    if style == 'solid':
        draw.line(path, fill=color, width=round(width))
        return
...
def _draw_shape_overlay(
    ...
    """Return RGB image with ROI outline drawn above the input image."""
...
class NodeROIViewWidget(NodeBaseWidget):
    ...
    def _apply_image(self, pil_image):
        self._apply_view_size(pil_image.width, pil_image.height)
        self._view.load_image(pil_image)

    def load_image(self, pil_image):
        if threading.current_thread() is threading.main_thread():
            self._apply_image(pil_image)
        else:
...
class ROIMaskNode(BaseExecutionNode):
    ...
    def evaluate(self):
        ...
        cropped[~mask_bool] = 0
        self.output_values['cropped_image'] = ImageData(
            payload=Image.fromarray(cropped, 'RGB'))
...
class CropNode(BaseExecutionNode):
    ...
    def evaluate(self):
        ...
        img_id = id(pil_in)
        if img_id != self._last_img_id:
            display = pil_in if pil_in.mode in ('RGB', 'L') else pil_in.convert('RGB')
            self._crop_widget.load_image(display)
            self._last_img_id = img_id
        ...
        self.set_progress(70)
        cropped = pil_in.crop((left, top, right, bottom))
...
def _draw_text_overlay(pil_image, shape_data: dict):
    """Draw a standalone text annotation on a PIL image."""
...
def _draw_curve_overlay(pil_image, shape_data: dict, *,
                        line_color=(0, 220, 220), line_width=2.0,
                        line_style='solid',
                        label_text='', label_x_offset=8.0,
                        label_y_offset=-8.0, label_font_size=12.0):
    """Draw a cubic Bézier curve with arrowhead on a PIL image."""
    from PIL import ImageDraw
...
class DrawShapeNode(BaseExecutionNode):
    ...
    def evaluate(self):
        ...
        if mask_port:
            for cp in mask_port.connected_ports():
                ...
                if isinstance(mdata, MaskData):
                    mask_bool = np.array(mdata.payload.convert('L')) > 0
        ...
        if label_port and label_port.connected_ports():
            ...
            if isinstance(ldata, LabelData):
                if ldata.image is not None:
                    _label_overlay_pil = ldata.image
        ...
        if pil_in is None and not mask_arrays:
            return False, "Connect an image, a mask, or both"

        if pil_in is None and mask_arrays:
            from PIL import Image as _PIL
            h, w = mask_arrays[0].shape
            pil_in = _PIL.new('RGB', (w, h), (0, 0, 0))
        ...
        out = pil_in.convert('RGB') if pil_in.mode != 'RGB' else pil_in.copy()
        ...
        if _label_overlay_pil is not None:
            ...
            if lbl_rgb.size != out.size:
                lbl_rgb = lbl_rgb.resize(out.size, _PILI.NEAREST)
            ...
            ov_pil = _PILI.fromarray(overlay, 'RGBA')
            out = _PILI.alpha_composite(out.convert('RGBA'), ov_pil).convert('RGB')
        ...
        if not no_preview:
            ...
            elif pil_in.mode in ('RGB', 'L'):
                display = pil_in
            else:
                display = pil_in.convert('RGB')
            self._draw_widget.load_image(display)
            if draw_shapes:
        ...
        self.output_values['image'] = ImageData(payload=out)
        self.set_progress(100)
        return True, None
...
class MaskEditorWidget(NodeBaseWidget):
    ...
    def _render(self):
        ...
        if self._bg_pil is not None:
            bg = np.array(self._bg_pil.convert('RGB').resize((w, h)))
        else:
...
class MaskEditorNode(BaseExecutionNode):
    ...
    def _on_mask_changed(self):
        ...
        if result is not None:
            pil = _PIL.fromarray((result.astype(np.uint8) * 255), 'L')
            self.output_values['mask'] = MaskData(payload=pil)
        else:
            self.output_values['mask'] = MaskData(payload=_PIL.new('L', (1, 1), 0))
        self.mark_clean()
        for out_port in self.outputs().values():
            for in_port in out_port.connected_ports():
                dn = in_port.node()
    ...
    def evaluate(self):
        ...
        if mask_port and mask_port.connected_ports():
            ...
            if isinstance(data, MaskData):
                mask_arr = np.array(data.payload.convert('L')) > 0

        if mask_arr is None and bg_pil is None:
        ...
        self._editor.set_input(mask_arr, bg_pil)
