...
class NodeAddedCmd(QtGui.QUndoCommand):
    ...
    def undo(self):
        ...
        scene = self.node.view.scene()
        rect = self.node.view.sceneBoundingRect() if scene else None
...
class NodesRemovedCmd(QtGui.QUndoCommand):
    ...
    def redo(self):
        ...
        for node in self.nodes:
            ...
            # Capture bounding rect before removal so we can force repaint
            scene = node.view.scene()
            rect = node.view.sceneBoundingRect() if scene else None
            node.view.delete()
            # Force repaint of the area — works around Qt6 stale artifact bug
            if scene and rect:
                scene.update(rect)
