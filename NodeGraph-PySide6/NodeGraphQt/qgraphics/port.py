...
class PortItem(QtWidgets.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        if self._hovered:
            # Hover: bright border, translucent fill, slightly expanded.
            pen = QtGui.QPen(border_color.lighter(140), 2.2)
            painter.setPen(pen)
            painter.setBrush(
                QtGui.QColor(color.red(), color.green(), color.blue(), 80))
            painter.drawEllipse(port_rect.adjusted(-1, -1, 1, 1))
        elif self.connected_pipes:
            # Connected: solid filled circle.
            pen = QtGui.QPen(border_color, 1.5)
            painter.setPen(pen)
            painter.setBrush(color)
            painter.drawEllipse(port_rect)
        else:
            # Disconnected: solid edge, semi-transparent fill.
            pen = QtGui.QPen(color, 1.5)
            painter.setPen(pen)
            fill = QtGui.QColor(color.red(), color.green(), color.blue(), 100)
