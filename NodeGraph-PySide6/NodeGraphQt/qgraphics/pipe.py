...
class PipeItem(QtWidgets.QGraphicsPathItem):
    ...
    def paint(self, painter, option, widget):
        ...
        # glow behind active / highlighted pipes.
        if self._active or self._highlight:
            glow_pen = QtGui.QPen(pen)
            glow_color = QtGui.QColor(pen.color())
            glow_color.setAlpha(45)
            glow_pen.setColor(glow_color)
            glow_pen.setWidthF(pen.widthF() + 6)
    ...
    def _draw_direction_pointer(self):
        ...
        # When nodes are very close, the bezier curve can loop back on itself
        # causing the sample points to give a flipped direction. Fall back to
        # the direct output→input vector in that case.
        if abs(dx) < 0.1 and abs(dy) < 0.1:
            out_pos = self._output_port.scenePos()
            in_pos = self._input_port.scenePos()
            dx = in_pos.x() - out_pos.x()
            dy = in_pos.y() - out_pos.y()
    ...
    def _draw_path_horizontal(self, start_port, pos1, pos2, path):
        ...
        if self.viewer_pipe_layout() == PipeLayoutEnum.CURVED.value:
            ...
            # Blender-style: tangent proportional to distance, no upper cap.
            _MIN = 80.0
            tangent = max(_MIN, horiz_dist * 0.5)

            # Backward connection (output right of input): extend tangent so
            # the curve loops cleanly around rather than doubling back tightly.
            is_backward = (
                (start_port.port_type != PortTypeEnum.IN.value and dx < 0) or
                (start_port.port_type == PortTypeEnum.IN.value and dx > 0)
            )
            if is_backward:
                tangent += dy * 0.25 + _MIN
    ...
    def activate(self):
        self._active = True
        # Use a brightened version of the port color for hover
        base = self._port_color()
        if len(base) == 3:
            base = (*base, 255)
        bright = tuple(min(255, int(c * 1.6)) for c in base[:3]) + (base[3],)
    ...
    def highlight(self):
        ...
        # Use a brightened version of the port color for highlighting
        base = self._port_color()
        if len(base) == 3:
            base = (*base, 255)
        bright = tuple(min(255, int(c * 1.4)) for c in base[:3]) + (base[3],)
        self.set_pipe_styling(
            color=bright,
            width=2,
            style=PipeEnum.DRAW_TYPE_DEFAULT.value
    ...
    def _port_color(self):
        """Return the data-type color from the connected ports, or default."""
        default = PortEnum.COLOR.value  # (120, 130, 140, 255)
        for port in (self._output_port, self._input_port):
            if port and hasattr(port, 'color'):
                pc = port.color
                if pc:
                    norm = (*pc, 255) if len(pc) == 3 else tuple(pc)
                    if norm != default:
                        return norm
        return self.color
    ...
    def reset(self):
        ...
        self._active = False
        self._highlight = False
        color = self._port_color()
        self.set_pipe_styling(color=color, width=2, style=self.style)
        self._draw_direction_pointer()
