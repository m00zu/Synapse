...
# ---------------------------------------------------------------------------
# Category → title-strip color mapping
# ---------------------------------------------------------------------------
_CATEGORY_COLORS = {
    'nodes.io':                      (52, 152, 219),    # Blue
    'nodes.image_process':           (46, 204, 113),    # Green
    'nodes.analysis':                (231, 76, 60),     # Red-coral
    'nodes.dataframe':               (52, 152, 219),    # Blue
    'nodes.data':                    (52, 152, 219),    # Blue
...
def _get_category_color(type_str):
    """Return (r, g, b) category color by matching progressively shorter
    prefixes of the node's type_ string."""
    # type_ looks like 'nodes.image_process.color.ColorSplitNode'
    # strip the class name at the end
    parts = type_str.rsplit('.', 1)[0] if '.' in type_str else type_str
    while parts:
        if parts in _CATEGORY_COLORS:
            return _CATEGORY_COLORS[parts]
        if '.' in parts:
            parts = parts.rsplit('.', 1)[0]
        else:
            break
    return (80, 90, 100)  # neutral default
...
class NodeItem(AbstractNodeItem):
    ...
    def _paint_horizontal(self, painter, option, widget):
        ...
        # drop shadow (painted manually — no QGraphicsEffect overhead).
        painter.setPen(QtCore.Qt.NoPen)
        for i in range(1, 5):
            shadow_rect = rect.adjusted(-i * 0.5, -i * 0.3, i * 0.5, i * 0.8)
            painter.setBrush(QtGui.QColor(0, 0, 0, max(0, 35 - i * 8)))
        ...
        # category accent line (thin colored line along top edge).
        cat_color = _get_category_color(self._properties.get('type_', ''))
        accent_pen = QtGui.QPen(QtGui.QColor(*cat_color), 2.0)
        accent_pen.setCapStyle(QtCore.Qt.RoundCap)
        painter.setPen(accent_pen)
        inset = radius
        painter.drawLine(
            QtCore.QPointF(rect.left() + inset, rect.top() + 1.0),
            QtCore.QPointF(rect.right() - inset, rect.top() + 1.0))
        painter.setPen(QtCore.Qt.NoPen)
        ...
        _viewer = self.viewer()
        _cosmetic = _viewer is not None and _viewer.get_zoom() < 0.0

        if self.selected:
            glow_color = QtGui.QColor(*NodeEnum.SELECTED_BORDER_COLOR.value)
            # outer glow layers
            for i in range(3, 0, -1):
                ga = int(glow_color.alpha() * 0.15 * (4 - i))
                gc = QtGui.QColor(glow_color.red(), glow_color.green(),
                                  glow_color.blue(), ga)
                pen = QtGui.QPen(gc, 0.8 + i * 0.6)
                pen.setCosmetic(_cosmetic)
                painter.setPen(pen)
                painter.setBrush(QtCore.Qt.NoBrush)
                painter.drawRoundedRect(
                    border_rect.adjusted(-i * 0.4, -i * 0.4,
                                         i * 0.4, i * 0.4),
                    radius, radius)
            # crisp inner border
            pen = QtGui.QPen(glow_color, 1.5)
            pen.setCosmetic(_cosmetic)
            painter.setPen(pen)
            painter.setBrush(QtCore.Qt.NoBrush)
    ...
    def mouseReleaseEvent(self, event):
        ...
        Args:
            event (QtWidgets.QGraphicsSceneMouseEvent): mouse event.
        """
        if bool(event.modifiers() & QtCore.Qt.KeyboardModifier.AltModifier):
            event.ignore()
            return
    ...
    def _calc_size_horizontal(self):
        ...
        if widget_width:
            width = max([width, widget_width + port_text_width + port_width + side_padding])
    ...
    def _align_widgets_horizontal(self, v_offset):
        ...
        for widget in self._widgets.values():
            ...
            if not inputs:
                x = max(rect.left() + 10, x)
            elif not outputs:
                x = min(rect.right() - widget_rect.width() - 10, x)
