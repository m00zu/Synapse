    ...
    def setTitleAlign(self, align="center"):
        ...
        style_dict = {
            ...
            "QGroupBox::title": {
                ...
                "color": "rgba({0}, {1}, {2}, 210)".format(*text_color),
                "padding": "0px",
            },
        }
        if self.title():
...
class NodeBaseWidget(QtWidgets.QGraphicsProxyWidget):
    ...
    def mousePressEvent(self, event):
        """
        Re-implemented to deselect the parent node and stop the event from
        propagating to the parent node item.
        """
        if self.node:
            self.node.set_selected(False)
        self.setFocus()
        event.accept()
    ...
    def mouseReleaseEvent(self, event):
        event.accept()
        super(NodeBaseWidget, self).mouseReleaseEvent(event)

    def mouseDoubleClickEvent(self, event):
        event.accept()
        super(NodeBaseWidget, self).mouseDoubleClickEvent(event)
    ...
    def set_custom_widget(self, widget):
        ...
        if self._label:
            ...
            group.setMinimumWidth(max(160, text_width + 40))
...
class _ProxyAwareComboBox(QtWidgets.QComboBox):
    """QComboBox that raises its parent QGraphicsProxyWidget z-order while
    the dropdown popup is visible, preventing sibling proxy widgets from
    overlapping the popup list."""

    def showPopup(self):
        proxy = self._find_proxy()
        if proxy is not None:
            self._saved_z = proxy.zValue()
            proxy.setZValue(Z_VAL_NODE_WIDGET + 999)
        super().showPopup()
    ...
    def hidePopup(self):
        try:
            super().hidePopup()
            proxy = self._find_proxy()
            if proxy is not None and hasattr(self, '_saved_z'):
                proxy.setZValue(self._saved_z)
        except RuntimeError:
    ...
    def _find_proxy(self):
        ...
        if proxy is not None:
            return proxy
        # PySide6: graphicsProxyWidget() may return None for child widgets.
        # Walk up the parent chain to find the proxy.
        w = self.parentWidget()
        while w is not None:
...
class NodeLineEdit(NodeBaseWidget):
    ...
    def __init__(self, parent=None, name="", label="", text="", placeholder_text=""):
        ...
        # Prevent key events from bubbling up to the NodeGraph view (which deletes the node)
        ledit.installEventFilter(self)

    def eventFilter(self, obj, event):
        if event.type() == QtCore.QEvent.Type.KeyPress:
            if event.key() in (QtCore.Qt.Key.Key_Backspace, QtCore.Qt.Key.Key_Delete):
                # Only let the line edit handle it, do not propagate up
                obj.keyPressEvent(event)
                return True
            elif event.key() in (QtCore.Qt.Key.Key_Escape, QtCore.Qt.Key.Key_Enter, QtCore.Qt.Key.Key_Return):
                obj.clearFocus()
        return super(NodeLineEdit, self).eventFilter(obj, event)
    ...
    def eventFilter(self, obj, event):
        if event.type() == QtCore.QEvent.Type.KeyPress:
            if event.key() in (QtCore.Qt.Key.Key_Backspace, QtCore.Qt.Key.Key_Delete):
                ...
                obj.clearFocus()
        return super(NodeCheckBox, self).eventFilter(obj, event)
...
class NodeImageShow(NodeBaseWidget):
    """
    Displays as a ``QLabel`` in a node.

    .. inheritance-diagram:: NodeGraphQt.widgets.node_widgets.NodeImageShow
        :parts: 1

    .. note::
        `To embed a` ``QLabel`` `in a node see func:`
        :meth:`NodeGraphQt.BaseNode.add_image`
    """

    def __init__(self, parent=None, name="", label="", image=None):
        super(NodeImageShow, self).__init__(parent, name, label)
        self.setZValue(Z_VAL_NODE_WIDGET + 1)
    ...
    def get_value(self):
        ...
        Returns:
            QPixmap: current image.
        """
        return self.get_custom_widget().pixmap()

    def set_value(self, image=None):
        """
        Sets the current image.

        Args:
            image (QPixmap): new image.
        """
        self.get_custom_widget().setPixmap(image)
