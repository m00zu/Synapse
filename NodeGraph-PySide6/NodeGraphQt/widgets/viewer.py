...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        for itm in items:
            if isinstance(itm, PipeItem):
                ...
                # if we click on a widget, we don't want to select the node.
                if any(isinstance(i, QtWidgets.QGraphicsProxyWidget) for i in items):
                    continue
        ...
        # show selection marquee with modifiers, else set pan flag.
    ...
    def mouseMoveEvent(self, event):
        ...
        elif self.MMB_state or (self.LMB_state and self.ALT_state) or getattr(self, '_LMB_panning', False):
        ...
        if not self.ALT_state:
            if self.SHIFT_state or self.CTRL_state:
                if not self._LIVE_PIPE.isVisible():
                    self._cursor_text.setPos(self.mapToScene(event.position().toPoint()))
    ...
    def wheelEvent(self, event):
        ...
        if isinstance(item, QtWidgets.QGraphicsProxyWidget):
            super(NodeViewer, self).wheelEvent(event)
            if event.isAccepted():
                return
        ...
        try:
            ...
            # Pan the view if no zoom modifiers are pressed (Ctrl/Cmd). 
            # Note: macOS pinch-to-zoom automatically sends the ControlModifier.
            if not (event.modifiers() & (QtCore.Qt.KeyboardModifier.ControlModifier | QtCore.Qt.KeyboardModifier.AltModifier)):
                self._set_viewer_pan(-delta_x, -delta_y)
                event.accept()
                return
    ...
    def keyPressEvent(self, event):
        ...
        """
        self.ALT_state = bool(event.modifiers() & QtCore.Qt.KeyboardModifier.AltModifier)
        self.CTRL_state = bool(event.modifiers() & QtCore.Qt.KeyboardModifier.ControlModifier)
        self.SHIFT_state = bool(event.modifiers() & QtCore.Qt.KeyboardModifier.ShiftModifier)

        # Todo: find a better solution to catch modifier keys.
        if (event.modifiers() & QtCore.Qt.KeyboardModifier.AltModifier) and (event.modifiers() & QtCore.Qt.KeyboardModifier.ShiftModifier):
            self.ALT_state = True
